---
title: '反本本主义：反经典崇拜、反榜单崇拜、反方法崇拜、反权威口味、反引用崇拜'
date: 2026-08-09
...
## 反本本主义：反经典崇拜、反榜单崇拜、反方法崇拜、反权威口味、反引用崇拜

这里说的“本本”，是把经典论文、benchmark、固定方法、权威口味和引用数当成现成答案。它们可以提供线索，不能代替对问题、证据和实际影响的判断。
...
用户应该排在第一位。即便信息本身带有泡沫或炒作，也不能因为这个标签就直接过滤；只要它和用户正在关心的事情有关，就应该说明它为什么出现、证据在哪里，再让用户决定要不要继续看。采编规则需要有弹性，预训练 LLM 也可以承担更多来源适配、初步筛选、归纳和持续跟进的工作。这里可以更积极地把权力交给 AI，同时保留原始链接、判断依据和纠错入口，让结果能被追溯、修改。
